    ...
    async def _host_judge(self, claims_mgr: ClaimsManager, round_num: int) -> list[dict]:
        ...
        for batch_index, claims_text in enumerate(claim_batches, start=1):
            ...
            prompt = (
                ...
                "不得用 OPEN 数量替代该语义判断。\n"
                "room status 必须与本次逐项 verdict 自洽，不得在 room reason 中"
                "把自己刚判定为 blocker 的 CONTINUE 改写成已密封条件。输出前必须复核"
                "每个 CONTINUE 的 reason、missing、needs_agents 和 "
                "allow_unknown_progress：\n"
                "- 只有当 material OPEN 截至当前确实不可获得、没有当前 Agent 或计算"
                "可以继续消解，且已被条件、区间、限制或更新触发器完整封装，并明确返回 "
                "allow_unknown_progress=true 时，room 才可在该 OPEN 存在时判 "
                "CONVERGED；room reason 必须说明其 SEALED_CONDITIONAL 边界。\n"
                "- 若 material CONTINUE 返回 allow_unknown_progress=false，表示 Host "
                "自己判定它仍是 blocker，room 必须判 NOT_CONVERGED 并保留定向路由。\n"
                "- 若 CONTINUE 的 reason 或 missing 表明当前 Agent 的交叉审阅、统一参数、"
                "模型对照、计算复核或现有可得来源仍可消解分歧，room 必须判 "
                "NOT_CONVERGED；不得把当前可完成的工作描述成未来不可得 UNKNOWN。\n"
                "- 若 material Challenger challenge 尚无针对该已记录挑战的具体后续"
                "结构化回应，且 verdict 为 CONTINUE/allow_unknown_progress=false，"
                "room 必须判 NOT_CONVERGED 并路由给能回应的 Agent。\n"
                "这是一致性语义复核，不是 OPEN 数量门槛、固定轮次门槛或运行时业务裁决；"
                "狭窄非阻塞 claim 以及真正已条件密封的未来 UNKNOWN 仍可自然完成。\n"
